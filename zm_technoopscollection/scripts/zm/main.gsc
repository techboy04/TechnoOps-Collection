...
//	create_dvar("gungame_ladder", 1);
	...
	precacheshader("hud_player_icon");
	...
	replacefunc(maps\mp\zombies\_zm_spawner::zombie_damage, ::zombie_damage);
	replacefunc(maps\mp\zombies\_zm_spawner::zombie_damage_ads, ::zombie_damage_ads);
	...
	else
	{
		...
		if(getDvarInt("gamemode") != 3 && getDvarInt("gamemode") != 4 && getDvarInt("gamemode") != 5 && getDvarInt("gamemode") != 7 && getDvarInt("gamemode") != 8)
		{
		...
		else
		{
			...
			else if (getDvarInt("gamemode") == 7)
			{
				...
				level.check_for_instakill_override = ::check_player_flowstate;
			}
			else if (getDvarInt("gammeode") == 8)
			{
				replacefunc(maps\mp\zombies\_zm::round_wait, ::round_wait_exfil);
				replacefunc(maps\mp\zombies\_zm::round_over, ::new_round_over);
				replacefunc(maps\mp\zombies\_zm::round_think, ::new_round_think);
...
//    level thread featuresList();
	...
	if( getDvarInt("gamemode") != 0)
	{
		...
		{
			level.zombiesnacksstarted = 0;
			level.energy_bar_mode = true;
			level.totalsnacks = 0;
			level.healthdraining = true;
			level.enable_flowstate = true;
			level thread introHUD();
		}
		else if(getDvarInt("gamemode") == 8)
		{
			level.gumgamestarted = 0;
		...
		if (getDvarInt("gamemode") != 0)
		{
			if(getDvarInt("gamemode") != 2 && getDvarInt("gamemode") != 4 && getDvarInt("gamemode") != 6 && getDvarInt("gamemode") != 8)
			{
...
onPlayerConnect()
{
    for(;;)
    {
		...
		if (getDvarInt("gamemode") == 1)
		{
			player.progmax = getdvarint("gungame_max_score");
			player.weaponprog = 0;
...
//			player thread moveCheckHUD();
		...
		{
			player.flowstate_killstreak = 0;
			player.flowstate_active = false;
		}
		else if (getDvarInt("gamemode") == 8)
		{
			player thread gumgame_loop();
		}
		
		player thread respawnPlayer();
		player thread nuke_earthquake();
		if(getDvarInt("do_revived_sound") == 1)
		{
...
onPlayerSpawned()
{
    ...
    for(;;)
    {
		...
		else
		{
			...
			{
				self thread snacks_drain_health();
				self thread player_flowstate_fx();
				self thread zombie_drops_snacks_check();
				self thread player_flowsstate_status();
			}
		}
    }
}
...
init_dvars()
{
	...
	create_dvar("enable_match_timer", 0);
	...
	create_dvar("tranzit_alpha_round_end", 1);
	create_dvar("do_revived_sound", 0);
...
init_player_things()
{
	...
	if(self.firstsetup == 0)
	{
		...
		if(getDvarInt("ptr_mode") == 1)
		{
			self iprintln("Welcome to the Public Test Region!");
			wait 2;
			self iprintln("This is made to test features before an update!");
			wait 2;
			self iprintln("If anything isnt working, feel free to leave feedback in our Discord server!");
			wait 2;
...
spawnRaygun(x,y,z,angle)
{
	...
	gun = "emp_grenade_zm";
	gunmodel = "t6_wpn_grenade_emp_world";
...
enemycounter_hud()
{
	...
	if(getDvarInt("zombie_counter_look") == 0)
	{
		return;
	}
	else if(getDvarInt("zombie_counter_look") == 1)
	{
		align = "bottom_center";
	}
	else if(getDvarInt("zombie_counter_look") == 2)
	{
		align = "top_right";
	}
	...
	if(align == "bottom_center")
	{
		alignx = "center";
		aligny = "bottom";
		horzalign = "user_center";
		vertalign = "user_bottom";
		x = 0;
		y = 0;
	}
	if(align == "top_right")
	...
	enemycounter.alignx = alignx;
	enemycounter.aligny = aligny;
	enemycounter.horzalign = horzalign;
	enemycounter.vertalign = vertalign;
	enemycounter.x = x - 2;
	enemycounter.y = y - 20;
	...
	if(getDvarInt("zombie_counter_look") == 1)
	{
		enemycounter.label = &"^2Enemies Left: ^7";
	}
	else if(getDvarInt("zombie_counter_look") == 2)
	{
		enemycounter.label = &"";
	...
	if(getDvarInt("zombie_counter_look") == 2)
	{
		enemycounter_icon = newhudelem();
		enemycounter_icon thread removeHUDEndGame();
		enemycounter_icon.alignx = alignx;
		enemycounter_icon.aligny = aligny;
		enemycounter_icon.horzalign = horzalign;
		enemycounter_icon.vertalign = vertalign;
		enemycounter_icon.x = x - 14;
		enemycounter_icon.y = y - 22;
		enemycounter_icon.fontscale = 1;
		enemycounter_icon.alpha = 1;
		enemycounter_icon.color = ( 1, 1, 1 );
		enemycounter_icon.hidewheninmenu = 1;
		enemycounter_icon.foreground = 1;
		enemycounter_icon setshader( "hud_zombie_icon", 25, 20 );
	}
...
init_exfil()
{
	if(getDvarInt("guided_mode") == 1 && if_current_map_has_guided())
	{
		return;
	}
...
health_bar_hud()
{
	...
	{
		health_text = newclienthudelem( self );
		health_text setText("Energy");
		health_text.alignx = "user_left";
...
player_hitmarker()
{
	if(isDefined(self.isbot) && self.isBot == 1)
	{
		return;
	}
	self thread damagehitmarker();
	self thread killmark();
}

damagehitmarker()
{
    self thread startwaiting();
	self.dohitmarkerred = false;
	self.dohitmarker = false;
...
hitmarker_zombie_kill(iscrit)
{
	if(self.dohitmarker != true)
	{
		...
		self.hitmarker setshader( "damage_feedback", 32, 60 );
		self.hitmarker.color = ( 1, 0, 0 );
		self.hitmarker.alpha = 1;
		self.hitmarker fadeovertime( 1 );
		self.hitmarker.alpha = 0;
		self.hitmarker scaleovertime( 0.25, 24, 48);
		wait 0.25;
		self.dohitmarkerred = false;
		self.dohitmarker = false;
	}
...
hitmarker_zombie_hit()
{
	self notify ("reset_hitmarker");
	self endon ("reset_hitmarker");
	self playlocalsound(getsound(1));
	if (self.dohitmarkerred == false && self.dohitmarker == false)
	{
...
hitmark()
{
    ...
    for(;;)
    {
		...
		if( isplayer( attacker ) )
        {
			if( isalive( self ))
			{
				attacker thread hitmarker_zombie_hit();
				if(getDvarInt("hitmarker_debug") == 1)
				{
					attacker iprintln ("Bullet Hit!");
				}
			}
        }
    }
...
killmark()
{
	...
	for(;;)
	{
		...
		{
			self thread hitmarker_zombie_kill(1);
			if(getDvarInt("hitmarker_debug") == 1)
			{
				self iprintln ("Crit Kill!");
			}
		}
		else if(isdefined( zombie ) && isdefined( zombie.damagemod ) && zombie.damagemod == "MOD_MELEE")
		{
			self thread hitmarker_zombie_kill(1);
			if(getDvarInt("hitmarker_debug") == 1)
			{
				self iprintln ("Melee Kill!");
			}
		}
		else
		{
			self thread hitmarker_zombie_kill();
		}
	}
}
...
getsound(type)
{
	if(type == 1)
	{
		...
		else if (getDvarInt("hit_sound") == 7)
		{
			return "hit_mwog";
		}
		else if (getDvarInt("hit_sound") == 8)
		{
			return "hit_bo7";
		}
	}
	else if (type == 2)
	{
		...
		else if (getDvarInt("kill_sound") == 7)
		{
			return "kill_mwog";
		}
		else if (getDvarInt("kill_sound") == 8)
		{
			return "kill_bo7";
		}
	}
	...
	{
		if(getDvarInt("downed_sound") == 1)
		{
			return "downed_bo4";
		}
		else if (getDvarInt("downed_sound") == 2)
		{
			return "downed_cw";
		}
		else if (getDvarInt("downed_sound") == 3)
		{
			return "downed_mw";
		}		
	}
	...
	{
		if(getDvarInt("crit_sound") == 1)
		{
			return "crit_bo7";
		}
		else if(getDvarInt("crit_sound") == 2)
		{
			return "crit_mw";
		}
	}
...
player_upgradedperks()
{
	...
	self thread tombstone_solo_ability();
...
custom_retain_array()
{
	...
	for(;;)
	{
		...
		foreach(perk in self.perks_active)
		{
			...
			{
				if(!isinarray( self.saved_perks, perk ))
				{
					self.saved_perks[self.saved_perks.size] = perk;
				}
			}
		}
	}
...
tombstone_solo_ability()
{
	self endon ("disconnect");
	self.solo_tombstones = 3;
	for(;;)
	{
		self waittill_any("player_revived");
		if(isinarray( self.saved_perks, "specialty_scavenger" ) && !self is_gobble_active("aftertaste"))
		{
			if(self.solo_tombstones > 0 || level.players.size != 1)
			{
				self.dovghud = 0;
				foreach(i in self.saved_perks)
				{
					if(i != "specialty_scavenger")
					{
						self give_perk( i, 0);
					}
				}
...
//				player iprintln("Used patchnotes command");
				...
				if(isDefined(args[1]))
				{
					...
					switch(setting)
					{
						case "tpall":
							...
							{
								if(isDefined(bot.isBot) && bot.isBot == 1)
								{
			...
			case ".w":
			case ".d":
				if(getDvarInt("goof_banklaugh") == 1)
				{
					if(!isDefined(player.cheatertimes))
					{
						player.cheatertimes = 0;
					}
					if(player.cheatertimes < 4)
					{
						player.cheatertimes += 1;
						player goofy_cheater(player.cheatertimes);
					}
					else
					{
						player goofy_laugh_at_player();
					}
				}
				break;
			case ".printperks":
				foreach(perk in player.saved_perks)
				{
					player iprintln(perk);
				}
				break;
			case ".gobbleactive":
				if(isDefined(args[1]))
...
double_points_powerup_new( drop_item, player )
{
	...
	level.double_points_active = true;
    players = get_players();
	...
	level.double_points_active = false;
...
bleedout_bar_hud_updatebar(hud)
{
	...
	{
		newtime = getDvarInt("player_lastStandBleedoutTime") * 2;
		vars["bleedout_time"] = newtime;
	}
	else
	{
...
init_powerups_minigame()
{
	...
	else if (getDvarInt("gamemode") == 8)
	{
		...
		add_zombie_powerup( "double_points", "zombie_x2_icon", &"ZOMBIE_POWERUP_DOUBLE_POINTS", ::func_should_always_drop, 0, 0, 0, undefined, "powerup_double_points", "zombie_powerup_point_doubler_time", "zombie_powerup_point_doubler_on" );
		add_zombie_powerup( "carpenter", "zombie_carpenter", &"ZOMBIE_POWERUP_MAX_AMMO", ::func_should_always_drop, 0, 0, 0 );
		add_zombie_powerup( "fire_sale", "zombie_firesale", &"ZOMBIE_POWERUP_MAX_AMMO", ::func_should_always_drop, 0, 0, 0, undefined, "powerup_fire_sale", "zombie_powerup_fire_sale_time", "zombie_powerup_fire_sale_on" );
		add_zombie_powerup( "bonfire_sale", "zombie_pickup_bonfire", &"ZOMBIE_POWERUP_MAX_AMMO", ::func_should_never_drop, 0, 0, 0, undefined, "powerup_bon_fire", "zombie_powerup_bonfire_sale_time", "zombie_powerup_bonfire_sale_on" );
		add_zombie_powerup( "minigun", "zombie_pickup_minigun", &"ZOMBIE_POWERUP_MINIGUN", ::func_should_never_drop, 1, 0, 0, undefined, "powerup_mini_gun", "zombie_powerup_minigun_time", "zombie_powerup_minigun_on" );
		add_zombie_powerup( "free_perk", "zombie_pickup_perk_bottle", &"ZOMBIE_POWERUP_FREE_PERK", ::func_should_never_drop, 0, 0, 0 );
		add_zombie_powerup( "tesla", "zombie_pickup_minigun", &"ZOMBIE_POWERUP_MINIGUN", ::func_should_never_drop, 1, 0, 0, undefined, "powerup_tesla", "zombie_powerup_tesla_time", "zombie_powerup_tesla_on" );
...
cranked_timer()
{
	...
	if(getDvarInt("gamemode") == 6)
	{
		foreach(player in level.players)
		{
			...
			playfx( level._effect["mechanical explosion"], player.origin );
			earthquake( 0.15, 0.5, player.origin, 250 );
		}
	}
	else
	{
...
loop_start_cranked_time() //Attempt to fix the early round bug.
{
...
changeweapon(demoted)
{
	...
	{
		self weapon_give( level.weaponlist[self.weaponlevel], is_weapon_upgraded( level.weaponlist[self.weaponlevel] ), 0, 1 );
	}
}
...
end_game_minigame()
{
	...
	if(getDvarInt("endgame_restart_map") > 0)
	{
		foreach(player in level.players)
		{
			player iprintln("The match will not take you to the lobby! This is WIP.\nType disconnect in the console to leave!");
		}
	}
...
//    }
	...
	if(getDvarInt("endgame_restart_map") == 1)
	{
		MissionFailed();
	}
...
#/
		...
		if (getDvarInt("gamemode") == 2 || getDvarInt("gamemode") == 4 || getDvarInt("gamemode") == 6 || getDvarInt("gamemode") == 7)
		{
			if ( isdefined( level.no_end_game_check ) && level.no_end_game_check )
			{
				level thread last_stand_revive();
...
startHUDMessage()
{
	...
	{
		hud3 settext("Your Health slowly drains, collect dropped sparks to gain health.");
	}
	else if (getDvarInt("gamemode") == 8)
	{
...
turn_on_next_tier()
{
	...
	self.progmax = getdvarint("gungame_max_score");
}
    ...
    if ( !isdefined( level._supress_survived_screen ) )
    {
        for ( i = 0; i < players.size; i++ )
        {
            ...
            else
            {
				...
				if(isDefined(level.completedmodmainquest) && level.completedmodmainquest == true)
				{
				...
				{
					if (isDefined(level.exfilstarted) && level.exfilstarted == 1 && getDvarInt("enable_exfil") == 1)
					{
...
{
	if(getDvarInt("gamemode") != 0 && getDvarInt("gamemode") != 8 && getDvarInt("guided_mode") == 0)
	{
		return;
	}
...
if_current_map_has_guided()
{
	if(getDvarInt("guided_mode") == 1)
	{
		supported_maps = array("zm_transit", "zm_nuked");
...
loophealth()
{
	for(;;)
	{
		...
		self iprintln("Current Drops: " + level.totalsnacks);
		self iprintln("Max Drops: " + level.players.size * 5);
		self iprintln("Killstreak: " + self.flowstate_killstreak);
		wait 1;
	}
}
...
playerhealthregen()
{
    ...
    for (;;)
    {
        ...
        if ( self.health <= 1 && !level.energy_bar_mode)
        {
...
zombie_drops_snacks_check()
{
	...
	for(;;)
	{
		...
		{
			random = randomintrange(0,10);
			if(random >= 8)
			{
				if(level.totalsnacks <= level.players.size * 8)
				{
					level thread snack_drop(zomb.origin);
				}
			}
		}
	}
...
snack_drop(location)
{
	trigger = spawn( "trigger_radius", ( location ), 1, 50, 50 );
	level.totalsnacks += 1;
	trigger.snack_fx = spawn( "script_model", location );
	trigger.snack_fx setModel ("tag_origin");
	playfxontag(level._effect[ "powerup_on" ], trigger.snack_fx, "tag_origin");
	trigger thread snack_drop_timeout();
	for(;;)
	{
		trigger waittill( "trigger", i );
		if(isplayer(i) && i.health < i.maxhealth)
		{
			if(i.maxhealth - 35 > i.health)
			{
				i.health = i.health + 35;
			}
			else
			{
				i.health = i.maxhealth;
			}
			trigger notify ("snack_collected");
			level.totalsnacks -= 1;
			trigger.snack_fx delete();
			i maps\mp\zombies\_zm_audio::playerexert( "burp" );
			i notify ("burp");
			i setblur( 4, 0.1 );
			wait 0.1;
			i setblur( 0, 0.1 );
...
snack_drop_timeout()
{
	self endon ("snack_collected");
	self endon ("death");
	wait 30;
	level.totalsnacks -= 1;
	self.snack_fx delete();
	self delete();
}
...
check_player_flowstate(player)
{
	if(player.flowstate_active || level.zombie_vars[player.team]["zombie_insta_kill"] == 1)
	{
		return true;
	}
	else
...
player_flowsstate_status()
{
	self endon ("disconnect");
	self endon ("death");
	if(!level.enable_flowstate)
	{
		return;
	}
	for(;;)
	{
		...
		self.flowstate_killstreak += 1;
		if(self.flowstate_killstreak >= 10)
		{
			self.flowstate_active = true;
			self notify ("flowstate_on");
			self playlocalsound ("statue_activate");
			player_flowstate_damage_check();
			self playlocalsound ("statue_deactivate");
		}
		else
		{
			self.flowstate_active = false;
		}
	}
}
...
player_flowstate_fx()
{
	...
	for(;;)
	{
		self waittill ("flowstate_on");
		flowstate_overlay_flash();
		self._flowstate_overlay destroy();
	}
}

flowstate_overlay_flash()
{
	self._flowstate_overlay = newclienthudelem( self );
	self._flowstate_overlay.x = 0;
	self._flowstate_overlay.y = 0;
	self._flowstate_overlay setshader( "overlay_low_health", 640, 480 );
	self._flowstate_overlay.alignx = "left";
	self._flowstate_overlay.aligny = "top";
	self._flowstate_overlay.horzalign = "fullscreen";
	self._flowstate_overlay.vertalign = "fullscreen";
	...
	for(;;)
	{
		...
		self._flowstate_overlay.alpha = 0.2;
		wait 0.5;
		self._flowstate_overlay fadeovertime( 0.5 );
		self._flowstate_overlay.alpha = 0;
		wait 1;
		
		if(self.flowstate_active != true)
		{
			break;
		}
	}
...
player_flowstate_damage_check()
{
	...
	for(;;)
	{
		...
		if (attacker maps\mp\animscripts\zm_utility::is_zombie())
		{
			self.flowstate_killstreak = 0;
			self.flowstate_active = false;
			self notify ("flowstate_lost");
			break;
		}
	}
}

createPlayerIcon(index, ref)
{	
	player_icon = newclienthudelem( self );
	...
	player_icon.alpha = 1;
	player_icon setshader( "hud_player_icon", 2, 2 );
	player_icon setwaypoint( 1 );
...
nuke_earthquake()
{
	...
	for(;;)
	{
		...
		foreach(player in level.players)
		{
			earthquake( 1, 2, player.origin, 400 );
		}
	}
}

carpenter_earthquake()
{
	earthquake( 1, 2, self.origin, 100 );
}
...
play_revive_sound()
{
	...
	for(;;)
	{
		...
		self playlocalsound("mod_player_revive");
		reviver playlocalsound("mod_player_revive");
	}
}
...
give_bots_special_weapons()
{
	foreach(player in level.players)
	{
		if (isDefined(player.isBot) && player.isBot == 1)
		{
			weapons = array("ray_gun_zm","ray_gun_upgraded_zm","raygun_mark2_zm","raygun_mark2_upgraded_zm");
			
			chosen = randomintrange(0,weapons.size - 1);
			
			player giveweapon (weapons[chosen]);
			player SetSpawnWeapon(weapons[chosen]);
			player thread dc_give_player_all_perks();
		}
	}
}
...
chugabud_corpse_revive_icon( player )
{
    ...
    hud_elem.alpha = 1;
    hud_elem.archived = 1;
    hud_elem setshader( "hud_icon_exfil", 5, 5 );
...
a1b2c3()
{
	for(;;)
	{
		...
		if(level.round_number == 23)
		{
			foreach(player in level.players)
			{
				player thread show_big_message("Ha you are getting OLD!", "zmb_laugh_richtofen");
			}
		}
	}
}
...
zombie_death_points( origin, mod, hit_location, attacker, zombie, team )
{
	...
	if(isDefined(zombie.dont_give_points) && zombie.dont_give_points == 1)
		return;
...
zombie_damage( mod, hit_location, hit_origin, player, amount, team )
{
	...
	if(isDefined(self.dont_give_points) && self.dont_give_points == 1)
		return;
...
{
	goof_hud = newClientHudElem(self);
	self playlocalsound("goofy_laugh");
	goof_hud.alignx = "center";
	goof_hud.aligny = "middle";
	goof_hud.horzalign = "user_center";
	goof_hud.vertalign = "user_top";
	goof_hud.x += 0;
	goof_hud.y += 120;
	goof_hud.fontscale = 2;
	goof_hud.alpha = 1;
	goof_hud.color = ( 1, 1, 1 );
	goof_hud.hidewheninmenu = 1;
	goof_hud.foreground = 1;
	goof_hud setShader("hud_goof_catlaugh", 0, 0);
	
	goof_hud moveOvertime( 0.25 );
    goof_hud scaleovertime( 0.25, 64, 64);
    goof_hud.setscale = 2;
    wait 3.25;
    goof_hud moveOvertime( 0.15 );
    goof_hud.setscale = 5;
...
goofy_cheater(times)
{
	...
	if(times == 1)
	{
		self playlocalsound("goofy_cartman_1");
		time = 4;
		subtitle = "Hey Tom Brady. You wanna play the game or do you just\nwanna be known as a smug cheating bitch for the rest of your life?";
	}
	else if(times == 2)
	{
		self playlocalsound("goofy_cartman_2");
		time = 4;
		subtitle = "OK, I guess you just want to Tom Brady your way through the whole game.\nWhats the matter? Cant do it on your own?";
	}
	else if(times == 3)
	{
		self playlocalsound("goofy_cartman_3");
		time = 2;
		subtitle = "Is this fun for you, Tom Brady?";
	}
	else if(times == 4)
	{
		self playlocalsound("goofy_cartman_4");
		time = 2;
		subtitle = "You broke the rules rules rules...";
	}
	self thread sendsubtitletext("Cartman", 2, subtitle, "", time);
	goof_hud.alignx = "right";
	goof_hud.aligny = "top";
	goof_hud.horzalign = "user_right";
	goof_hud.vertalign = "user_top";
	goof_hud.x -= 120;
	goof_hud.y += 120;
	goof_hud.fontscale = 2;
	...
	goof_hud.hidewheninmenu = 1;
	goof_hud.foreground = 1;
	goof_hud setShader("hud_goof_cartman", 0, 0);
	
	goof_hud moveOvertime( 0.15 );
    goof_hud scaleovertime( 0.15, 128, 128);
    goof_hud.setscale = 2;
    wait time + 0.15;
    goof_hud moveOvertime( 1 );
    goof_hud.setscale = 2;
    goof_hud scaleovertime( 0.15, 0, 0);
	wait 0.15;
	goof_hud destroy();
