...
init() {
	...
	if(getDvarInt("experimental_trial") != 1)
	{
		return;
	}
...
TrialsSystem(CalculatedOrigin,SelectedModel, Origin, Angles, ActivatiorModel, ActivatiorOrigim, ActivatorAngles) {
	...
	TrialPodium_Player1 thread createRewardIcon(Origin[0] + CalculatedOrigin,0);
	TrialPodium_Player1 thread PodiumSetupTrigger(Origin[0] + CalculatedOrigin,4);
	TrialPodium_Player1 thread createRewardIcon(Origin[0] + CalculatedOrigin,4);
	...
	TrialPodium_Player2 thread createRewardIcon(Origin[1] + CalculatedOrigin,1);
	TrialPodium_Player2 thread PodiumSetupTrigger(Origin[1] + CalculatedOrigin,5);
	TrialPodium_Player2 thread createRewardIcon(Origin[1] + CalculatedOrigin,5);
	...
	TrialPodium_Player3 thread createRewardIcon(Origin[2] + CalculatedOrigin,2);
	TrialPodium_Player3 thread PodiumSetupTrigger(Origin[2] + CalculatedOrigin,6);
	TrialPodium_Player3 thread createRewardIcon(Origin[2] + CalculatedOrigin,6);
	...
	TrialPodium_Player4 thread createRewardIcon(Origin[3] + CalculatedOrigin,3);
	TrialPodium_Player4 thread PodiumSetupTrigger(Origin[3] + CalculatedOrigin,7);
	TrialPodium_Player4 thread createRewardIcon(Origin[3] + CalculatedOrigin,7);
		...
		if(level.ReaperTrialsActive == 0){
        	if(player UseButtonPressed()){
        		if(player.score < TrialsCost){
					...
					player playsound ("trials_start");
					level notify ("trials_start");
...
ChallengeHandler(Zones,Challenge){
	...
	{
		for(i = 0;i < players.size;i++){
			players[i] playlocalsound("clock_tick");
		}
		wait 1;
	}
	...
	for(i = 0;i < players.size;i++){
		...
		players[i] playlocalsound("trials_end");
		players[i] toggle_trial_challenge_hud();
	}
	level.ReaperTrialsActive = 0;
	level notify ("trials_over");
}
...
toggle_trial_challenge_hud() {
    ...
    if (isdefined(self.trials_show_challenge) && self.trials_show_challenge) {
		...
		horizalign = "user_right";
		vertalign = "user_center";
		
/////////////////////////////////////////////
        // Main background
        if (!isdefined(self.trials_bg)) {
            self.trials_bg = newclienthudelem(self);
            self.trials_bg.horzalign = horizalign;
            self.trials_bg.alignx = "left";
            self.trials_bg.vertalign = vertalign;
            self.trials_bg.aligny = "middle";
            self.trials_bg.x = x - sq_wide;
            self.trials_bg.y = y;
            self.trials_bg.sort = 1;
            self.trials_bg.foreground = false;
        ...
        if (!isdefined(self.trials_timer)) {
            ...
            self.trials_timer.horzalign = horizalign;
            self.trials_timer.alignx = "center";
            self.trials_timer.vertalign = vertalign;
            ...
            self.trials_timer.foreground = false;
        ...
        if (!isdefined(self.trials_challenge)) {
            ...
            self.trials_challenge.horzalign = horizalign;
            self.trials_challenge.alignx = "right";
            self.trials_challenge.vertalign = vertalign;
            ...
            self.trials_challenge.foreground = false;
...
draw_reward_alert(text) {
    ...
    if (!isdefined(self.trials_upgrade_shadow)) {
        ...
        self.trials_upgrade_shadow.foreground = false;
    ...
    if (!isdefined(self.trials_upgrade_bg)) {
        ...
        self.trials_upgrade_bg.foreground = false;
    ...
    if (!isdefined(self.trials_upgrade)) {
        ...
        self.trials_upgrade.foreground = false;
    ...
    self.trials_upgrade.alpha = 0;
    wait .25;
	self.trials_upgrade_shadow destroy();
	self.trials_upgrade destroy();
	self.trials_upgrade_bg destroy();
}
...
draw_trial_progress() {
    ...
    // Animation
    self playlocalsound("trials_tick");
	
	self green_glow(self.trials_challenge);
			...
			if(isdefined(GetPlayers()[Index])){
				if(isdefined(GetPlayers()[Index].ReaperTrialsCurrentMagic) && GetPlayers()[Index].ReaperTrialsCurrentMagic >= 25){
					if(level.script == "zm_transit")
						...
						playfx(level._effect[ "fx_alcatraz_elec_chair" ],FXOrigin - (17,0,15));// Too Lazy
					else if(level.script == "zm_buried")
						playfx( level._effect[ "character_fire_death_sm" ], FXOrigin);
					else if(level.script == "zm_highrise")
						playfx( level._effect[ "character_fire_death_sm" ], FXOrigin);
...
playtrialsmusic()
{
	...
	ent playloopsound( "mus_trials_loop", 5 );
}

stoptrialsmusic()
{
    level waittill_any( "end_game", "trials_over", "exfil_started");
    self stoploopsound( 0.5 );
    wait 0.5;
    self delete();
}

createRewardIcon(Origin,Index)
{
	while(!isDefined(level.players[Index]))
	{
		wait 0.1;
	}
	reward_icon = newClientHudElem(level.players[Index]);
    reward_icon.x = Origin[0];
    reward_icon.y = Origin[1];
	reward_icon.z = Origin[2] + 40;
	reward_icon.color = (1,1,1);
    reward_icon.alpha = 1;
    reward_icon setshader( "hud_icon_reward", 4, 4 );
    reward_icon setwaypoint( 1 );
	
	while(1)
	{
		if(level.players[Index].ReaperTrialsCurrentMagic >= 25)
		{
			reward_icon.alpha = 1;
		}
		else
