...
main()
{
	replacefunc(maps\mp\zombies\_zm_powerup_zombie_blood::zombie_blood_powerup, ::zombie_blood_powerup);
}
...
zombie_blood_powerup( m_powerup, e_player )
{
    ...
    if(level.gobblegums[e_player.gobblegum].id == "temporal_gift" && e_player.gobblegum_active == 1)
	{
		e_player.zombie_vars["zombie_powerup_zombie_blood_time"] = 60;
	}
	else
	{
    ...
    if ( !isdefined( e_player.early_exit ) )
		if(!(level.gobblegums[e_player.gobblegum].id == "idle_eyes" && e_player.gobblegum_active == 1))
			e_player.ignoreme = 0;
    else
