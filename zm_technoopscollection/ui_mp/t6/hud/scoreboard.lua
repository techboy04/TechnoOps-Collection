	...
	if ScoreboardWidget.mode == "theater" then
		...
		if not ScoreboardWidget.frontEndOnly then
			if CoD.isZombie == true then 
				StringTable[1] = GetSSGameType()
				StringTable[2] = " - "
				StringTable[3] = getSSMapName()
			else
...
getSSMapName = function()
	if Dvar.ui_mapname:get() == "zm_transit" then
		if Dvar.ui_gametype:get() == "zstandard" then
			if Dvar.ui_zm_mapstartlocation:get() == "town" then
				return "Town"
			elseif Dvar.ui_zm_mapstartlocation:get() == "farm" then
				return "Farm"
			elseif Dvar.ui_zm_mapstartlocation:get() == "transit" then
				return "Bus Depot"
			end
		else
			return "Tranzit"
