...
CoD.Loading.DidYouKnow.zombies = {
	...
	"When completing the Rampage Statue, it will have a reward for you!",
	"Bored of classic zombies? Try out Gun Game, Mystery Guns and Cranked!",
	"Bored of classic zombies? Try out Dead Light Green Light, and Sharpshooter!",
	"Some perks have additional affects! Speed Cola gives you fast hands and can build buildables quicker!",
	"Some perks have additional affects! Double Tap speeds Mystery Box and Barrier Rebuild!",
	"Some perks have additional affects! Staminup lets you run infinitely!",
	"Some perks have additional affects! Mulekick saves your third weapon!",
	"Some powerups have additional affects! With Full Ammo enabled, Max Ammo fills your magazine!",
	"Some powerups have additional affects! Carpenter will fully rebuild shield",
	"Some powerups have additional affects! With Useful Nuke enabled, the Nuke will give players points per zombie killed!",
	"On Tranzit, you can eliminate all the traveling with a small price with the Fast Travel!",
	"On Tranzit, there is a ladder in Town that connects to the road between Diner and Farm!",
	"Buildables that require a power source will turn on when the maps power is on! No Turbine Needed!",
	"Got a late joiner? The Global ATM lets you hand them points to give them a head start!",
	"Tranzit's Alpha Round End theme is on any Tranzit map!",
	"On Mob of the Dead and Nuketown, obtaining all the perks will give you infinite sprint.",
	"Shoutout to Sehteria for adding all zombie weapons and extra maps to all maps!",
...
LUI.createMenu.Loading = function (LocalClientIndex)
	...
	LoadingWidget.gametypeLabel:setText(GetGameType())
...
CoD.Loading.StartLoading = function (LoadingWidget, f8_arg1)
	LoadingWidget.mapNameLabel:setText(UIExpression.ToUpper(nil, Engine.Localize(getMapName())))
	LoadingWidget.mapLocationLabel:setText(Dvar.ls_mapname:get())
	LoadingWidget.gametypeLabel:setText(GetGameType())
...
GetGameType = function()
	if UIExpression.DvarInt(nil,"gamemode") == 0 then
		return "CLASSIC"
	elseif UIExpression.DvarInt(nil,"gamemode") == 1 then
		return "GUN GAME"
	elseif UIExpression.DvarInt(nil,"gamemode") == 2 then
		return "CRANKED"
...
getMapName = function()
	if Dvar.ui_mapname:get() == "zm_transit" then
		if Dvar.ui_gametype:get() == "zstandard" and Dvar.ui_zm_mapstartlocation:get() == "transit" then
			return "Bus Depot"
		elseif Dvar.ui_gametype:get() == "zstandard" and Dvar.ui_zm_mapstartlocation:get() == "farm" then
